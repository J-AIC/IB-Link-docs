...
##### サーバー設定
コンテキストサイズ、ホスト、ポート、スレッド数などのサーバー設定を構成できます。「Add Custom Argument」ボタンでカスタム引数を追加できます。設定を変更した後、「Save Server Settings」ボタンで保存します。「Command Preview」で、サーバーに渡されるパラメータを確認できます。
