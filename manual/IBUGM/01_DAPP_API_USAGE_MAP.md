...
|---|---|---|---|---|---|
| Chat | `src/assets/js/apiClient.js`（RAG Chat / /v1/models で到達性確認）<br>`src/api/IBLinkClient.js: chatCompletion()`（/chat/completions） | **LLM推論**: `http://localhost:8080/v1/chat/completions`<br>**Model list**: `http://localhost:8080/v1/models`<br>**8500/iblink 経路**: `http://localhost:8500/iblink/v1/chat/completions`（baseURLを8500にした場合） | `messages` を使用（system+user）<br>`stream` を考慮（ただし `apiClient.js` の `ragChatCompletions` は `stream:false`） | `503` の `Loading model` を検知して再試行する分岐あり | `manual/Dapp/d-josys/src/assets/js/apiClient.js` L5-L7, L102-L123, L227-L257, L288-L306<br>`manual/Dapp/d-josys/src/api/IBLinkClient.js` L274-L279 |
...
- **7000/realtime（SignalR / WebSocket）**: `http://localhost:7000/realtime` / `http://localhost:7000/iblink/v1/audio/health`（D-Retailで定数化）
